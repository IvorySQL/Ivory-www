---
sidebar_label: 'compatible conversion and comparision function'
sidebar_position: 1
---
...
### **Parameters**
```str```
	Input parameter including the following data types(any type).  
```fmt```
	Input format pararmeter,see format attachment for details.  
...
select hex_to_decimal('0x7ffffffffffffffff');
ERROR:  value "7ffffffffffffffff" is out of range!,the valid value of the input parameter is 0-0x7FFFFFFFFFFFFFFF
```
...
select to_binary_double('ad');
 ERROR:  invalid input syntax for type double precision: "ad"
select to_binary_double(1.234567891e+400::numeric);
 ERROR:  "" is out of range for type double precision
```
...
select to_binary_float('ab');
ERROR:  invalid input syntax for type real: "ab"

postgres=# select to_binary_float(1.234567891e+200::numeric);
...
select bin_to_num(1.2::real,2.3::numeric); 
 ERROR:  Invalid binary number,each paramter must evaluate to 0 or 1!
select bin_to_num('3'::bpchar);
 ERROR:  Invalid binary number,each paramter must evaluate to 0 or 1!
select bin_to_num(3.2::float8,1::int4);
 ERROR:  Invalid binary number,each paramter must evaluate to 0 or 1!
...
ERROR:  function to_multi_byte(numeric) does not exist
select to_multi_byte(1::int);
ERROR:  function to_multi_byte(integer) does not exist
```
...
select to_single_byte(1.2::int);
ERROR:  function to_single_byte(integer) does not exist
```
...
select to_date('-0-07-23 14:31:23', 'SYYYY-MM-DD HH24:MI:SS');
...
select to_timestamp('20fdfsd');
ERROR:  invalid input syntax for type double precision: "20fdfsd"
```
...
select to_yminterval('3 year');
ERROR:  parameter is invalid!
select to_yminterval('-01-02-04');
ERROR:  parameter is invalid!
```
...
select to_dsinterval('-P100DT');
ERROR:  parameter is invalid!
select to_dsinterval('-P1Y2M');
ERROR:  parameter is invalid!
```
...
select to_timestamp_tz('2005555');
ERROR:  date/time field value out of range: "2005555"
```
...
SELECT nanvl('NaN', 'aas');
ERROR:  invalid input syntax for type double precision: "aas"
```
